...
def calc_predicted_expectation(feature_function, train_data, feature_functions):
    predicted_expectation = 0
    for words, labels in train_data:    
        for i in range(1, len(labels)):
            predicted_expectation += (calc_prob_labels_given_words(labels, feature_functions , train_data) * sum([feature_function.apply(labels[i - 1], labels[i]) for i in range(1, len(labels))]))
    return predicted_expectation


def calc_prob_labels_given_words(labels, feature_functions, train_data):
    nominator = 1
...
def train(train_data, all_labels, features_functions):
    ...
    for _ in range(iterations):
        for feature_function in feature_functions:
            ...
            predicted_expectation = calc_predicted_expectation(feature_function, train_data,features_functions)
